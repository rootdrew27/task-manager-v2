...
export async function validateApiKeys(apiKeys: ApiKeys): Promise<ValidationResult> {
  ...
  if (apiKeys.openai.trim()) {
    ...
    validationPromises.push({service: 'openai', isValid: false, error: "OpenAI API Key Required"})
  }
  ...
  if (apiKeys.deepgram.trim()) {
    ...
    validationPromises.push({service: 'deepgram', isValid: false, error: "Deepgram API Key Required"})
  }
...
export async function saveConfiguration(data: SetupData): Promise<ConfigurationResult> {
  ...
  if (isValid) {
    ...
    console.log('API Keys validated for:', Object.keys(apiKeys).filter(key => apiKeys[key as keyof ApiKeys].trim()));
